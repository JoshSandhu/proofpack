...
def test_every_reason_code_has_at_least_one_corpus_case():
    ...
    joined in repair 2, ``template_mismatch`` in repair 3), 146 corpus files
    (``116``-``130`` joined in repair 2, ``131``-``140`` in repair 3, ``141``-``146`` in
    repair 4); every code is the
    ...
    assert len(checker.REASON_CODES) == 44 and len(CORPUS_FILES) == 146

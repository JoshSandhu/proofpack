...
Every value is produced by a seeded numpy Generator; nothing is hand-typed. Since build
day 9 the cohort generator itself lives in ``proofpack.synthetic`` (the sample-pack script
draws the same rows) and is re-exported here unchanged.
"""
...
from proofpack.synthetic import make_cohort  # noqa: F401 - re-exported to the tests
